...
from tgbot.models.user import TGUser as DbTGUser
...
def _load_translations(path: str = None) -> dict:
    ...
    try:
        return yaml.safe_load(
            Path(os.path.join(path)).read_text(encoding="utf-8")
        )
...
class TranslationMiddleware(BaseMiddleware):
    ...
    async def get_available_langs(self, obj, data) -> list:
        """Returns list of available languages"""
        return self.texts.get("available_langs", [])
    ...
    async def on_pre_process_callback_query(
        self, obj: CallbackQuery, data: dict
    ) -> Map:
        data["texts_original"] = Map(self.texts)
        data["texts"] = await self.reload_translations(obj, data)
